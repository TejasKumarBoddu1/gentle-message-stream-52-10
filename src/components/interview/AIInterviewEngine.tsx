...
const AIInterviewEngine: React.FC<AIInterviewEngineProps> = ({
  ...
  const handleAnswerSubmit = async () => {
    ...
    try {
      ...
      const avgScore = newHistory.reduce((sum, eval) => sum + eval.overallScore, 0) / newHistory.length;
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              {evaluationHistory.slice(-2).map((eval, index) => (
                <div key={index} className="p-3 border rounded-lg">
                  <div className="flex justify-between items-center mb-2">
                    <span className="font-medium">Answer {evaluationHistory.length - 1 + index}</span>
                    <Badge variant={eval.overallScore >= 70 ? "default" : "secondary"}>
